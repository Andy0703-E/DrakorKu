    ...
    <div className="min-h-screen container mx-auto px-6 py-10 max-w-[1400px]">
      <div className="mb-10">
        <h1 className="text-4xl font-black text-white mb-3">
          DRAMA <span className="text-primary">REKOMENDASI</span>
        </h1>
        <div className="h-1 w-24 bg-primary rounded-full" />
        {!loading && (
          <p className="text-gray-400 mt-3">{data.length} drama rekomendasi ditampilkan</p>
        )}
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-6">
            {data.length > 0 ? (
              data.map((item, i) => (
                <Card key={`${item.id || i}-${i}`} data={item} />
              ))
            ) : (
              <div className="col-span-full text-center py-20 text-gray-500">
                Tidak ada drama rekomendasi ditemukan.
              </div>
            )}
          </div>
